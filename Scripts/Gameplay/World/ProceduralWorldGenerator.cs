...
/// <summary>
/// Default implementation of <see cref="IWorldGenerator"/> using deterministic region templates
/// and lightweight procedural rules to produce locations and their connections.
/// </summary>
public sealed class ProceduralWorldGenerator : IWorldGenerator
{
    ...
    private readonly RegionLocationGenerator _regionLocationGenerator;

    /// <summary>
    /// Initializes a new instance of the <see cref="ProceduralWorldGenerator"/> class.
    /// </summary>
    /// <param name="random">Optional random source used for repeatable generation.</param>
    /// <param name="regionLocationGenerator">Optional location generator dependency.</param>
    public ProceduralWorldGenerator(Random? random = null, RegionLocationGenerator? regionLocationGenerator = null)
    {
        _random = random ?? new Random();
        _regionLocationGenerator = regionLocationGenerator ?? new RegionLocationGenerator(_random);
    }

    /// <inheritdoc />
    public WorldGenerationResult GenerateWorld()
    {
        var regions = CreateBaseRegions();
        ...
        return new WorldGenerationResult(regions, locations);
    }

    private static List<Region> CreateBaseRegions()
    {
    ...
    private static void InitializeRegionClimate(Region region)
    {
